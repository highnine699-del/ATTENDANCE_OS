...
function getSemesterWeeks(semInfo) {
    const totalWeeks = Number.parseInt(semInfo.lectureWeeks, 10) || 13;
    if (!semInfo.startDate) return { currentWeek: 1, weeksLeft: totalWeeks - 1, totalWeeks };

    const start = new Date(semInfo.startDate);
    if (Number.isNaN(start.getTime())) return { currentWeek: 1, weeksLeft: totalWeeks - 1, totalWeeks };
...
function projectEndPercentage(course, semInfo) {
    ...
    if (currentWeek <= 0) return calculatePercentage(attended, total);

    const lectPerWeek = Math.max(1, Math.round((total / Math.max(1, currentWeek)) * 10) / 10);
...
export function renderAnalytics(state, container) {
    ...
    const sorted = [...withData].sort(
        (a, b) => calculatePercentage(a.attended || 0, a.totalClasses || 0) - calculatePercentage(b.attended || 0, b.totalClasses || 0)
    );

    const weightedAvg = calculateWeightedAttendance(courses);
    const overallStatus = withData.length > 0 ? getStatus(weightedAvg, threshold) : 'no-data';
    const totalSafeSkips = withData.reduce(
        (sum, c) => sum + calculateSafeSkips(c.attended || 0, c.totalClasses || 0, threshold), 0
    );
    const belowThreshold = withData.filter(
        c => calculatePercentage(c.attended || 0, c.totalClasses || 0) < threshold
    );
    const unitsAtRisk = belowThreshold.reduce((sum, c) => sum + (c.units || 0), 0);

    const recoveryList = belowThreshold
        .map(c => {
            const pct = calculatePercentage(c.attended || 0, c.totalClasses || 0);
            const needed = calcLecturesNeeded(c.attended || 0, c.totalClasses || 0, threshold);
        ...
        <div class="analytics-module">
            ...
            <div class="analytics-stats-grid">
                <div class="analytics-stat-card">
                    ...
                    <span class="stat-value" style="color:${getStatusMeta(overallStatus).color}">${withData.length > 0 ? weightedAvg + '%' : '—'}</span>
                </div>
                <div class="analytics-stat-card">
