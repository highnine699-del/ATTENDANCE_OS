...
function getSemesterProgress(semesterInfo) {
    ...
    if (semesterInfo.lectureWeeks) {
        const totalWeeks = parseInt(semesterInfo.lectureWeeks) || 13;
...
export function renderDashboard(state, container) {
    ...
    const coursesWithData = courses.filter(c => c.totalClasses > 0);
    const overallStatus = coursesWithData.length > 0
        ? getStatus(weightedAvg, threshold)
        : 'no-data';
    ...
    const sortedCourses = [...courses]
        ...
        .sort((a, b) => calculatePercentage(a.attended || 0, a.totalClasses || 0) - calculatePercentage(b.attended || 0, b.totalClasses || 0));

    const atRisk = sortedCourses.filter(c => calculatePercentage(c.attended || 0, c.totalClasses || 0) < threshold);
    const allSafe = atRisk.length === 0;
    const critical = sortedCourses.filter(c => calculatePercentage(c.attended || 0, c.totalClasses || 0) < threshold - 25);
    ...
    if (skipSelect && skipResult) {
        function updateSkipAnswer() {
            ...
            if (!course) return;
            const safeSkips = calculateSafeSkips(course.attended || 0, course.totalClasses || 0, threshold);
            if (safeSkips > 0) {
